...
| `ResultRecord` | `experimentId, candidateId, replicateKdsM: number[]|null(M), konMInvSInv: number|null(M, M⁻¹·s⁻¹), koffPerS: number|null(M, s⁻¹), kdMeanM: number|null(M), rmseMaxSignalPct: number|null(M, contract), fitQualityReported: 'good'|'medium'|'poor'|null(M, contract), confidence: 'high'|'medium'|'low'|null(M, contract), controlOutcome: 'pass'|'fail'|'na'(M), measurements: Measurement[](M)` | **Contract-faithful BLI fields** (KD/k_on (M⁻¹·s⁻¹)/k_off (s⁻¹), replicate dispersion, `fit_quality`, `rmse_max_signal_pct`, `confidence`). Any non-contract metric would be explicitly labelled synthetic; none is used. |
