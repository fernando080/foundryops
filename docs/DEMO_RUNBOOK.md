...
4. **Approval boundary (1:45–2:25).** Show the **approval summary** (residues redacted) and the **canonical hash** chip that binds the exact server-side payload. `Create draft` is enabled; `Confirm & submit to lab` is padlocked. **Edit replicates** → the hash changes and the prior approval shows **INVALIDATED**. **Reissue approval**, then **Create draft** — exactly one deterministic draft is created.
