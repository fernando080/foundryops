...
const outputDir = process.env.AUDIT_OUTPUT_DIR ?? '/tmp/foundryops-pr2-audit'
...
try {
  ...
  await page.getByTestId('exact-payload-fields').waitFor()
  await page.getByTestId('approval-stage').screenshot({ path: join(outputDir, '04-exact-payload.png') })

  await page.getByTestId('edit-replicates').fill('2')
  await page.getByTestId('approval-invalidated').waitFor()
  await page.getByTestId('approval-stage').screenshot({ path: join(outputDir, '05-approval-invalidated.png') })
  ...
  await page.getByTestId('timeline-stage').screenshot({ path: join(outputDir, '06-timeline-audit.png') })
  ...
  await page.getByTestId('results-stage').screenshot({ path: join(outputDir, '07-results.png') })

  await page.getByTestId('generate-draft').click()
  await page.getByTestId('draft-evidence-chip').first().waitFor()
  await page.getByTestId('results-stage').screenshot({ path: join(outputDir, '08-evidence-backed-draft.png') })
} finally {
  await browser.close()
}
